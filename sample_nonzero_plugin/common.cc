...
bool readPGMFile(const std::string& path, uint8_t* data, int h, int w) {
  ...
  if (!ifs) {
    std::cout << "Failed to open PGM: " << path << std::endl;
    return false;
  }
  ...
  if (magic != "P5" && magic != "P2") {
    std::cout << "Unsupported PGM magic: " << magic << " (expect P5 or P2)" << std::endl;
    return false;
  }
  ...
  ifs >> maxval;
  if (W != w || H != h) {
    std::cout << "PGM size mismatch. Expect (" << w << "," << h << ") but got (" << W << "," << H
              << "). "
              << "Will read and crop/clip as needed." << std::endl;
  }
  if (maxval <= 0 || maxval > 65535) {
    std::cout << "Invalid maxval in PGM: " << maxval << std::endl;
    return false;
  }
