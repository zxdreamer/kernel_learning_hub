...
bool NonZeroPlugin::supportsFormatCombination(int32_t pos,
  ...
  std::cout << "complate supportsFormatCombination...." << std::endl;
...
int32_t NonZeroPlugin::getOutputShapes(DimsExprs const* inputs,
  ...
  outputs[1].nbDims = 0;
  std::cout << "complate getOutputShapes...." << std::endl;
  return 0;
}
...
int32_t NonZeroPlugin::enqueue(PluginTensorDesc const* inputDesc,
  ...
  if (!(type == nvinfer1::DataType::kHALF || type == nvinfer1::DataType::kFLOAT)) {
    std::cout << "Unsupported: Sample only supports DataType::kHALF "
                 "and DataType::FLOAT"
              << std::endl;
    return -1;
  }
  ...
  // 3. 调用kernel，填充结果
  std::cout << "mRowOrder: " << mRowOrder << std::endl;
  if (!mRowOrder) {
  ...
  std::cout << "complate enqueue...." << std::endl;
  return 0;
}
