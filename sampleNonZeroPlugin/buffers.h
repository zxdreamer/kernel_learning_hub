...
public:
  ...
  void copyInputToDeviceAsync(cudaStream_t stream) {
    for (auto& kv : mTensors) {
      ...
      if (t.isInput && t.h && t.d && t.bytes) {
        ASSERT_TRUE(cudaMemcpyAsync(t.d, t.h, t.bytes, cudaMemcpyHostToDevice, stream));
      }
    }
  }
  ...
  void copyOutputToHostAsync(cudaStream_t stream) {
    for (auto& kv : mTensors) {
      auto& t = kv.second;
      if (!t.isInput && t.h && t.d && t.bytes) {
        ASSERT_TRUE(cudaMemcpyAsync(t.h, t.d, t.bytes, cudaMemcpyDeviceToHost, stream));
      }
    }
  }
...
private:
  ...
  void init(const std::vector<int64_t>& ioVolumes) {
    ...
    if (static_cast<int>(ioVolumes.size()) != n) {
      samplesCommon::gLogWarning << "[BufferManager] ioVolumes size (" << ioVolumes.size()
                                 << ") != IOTensors (" << n << "). Will try best-effort."
                                 << std::endl;
    }
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (esz == 0) {
        samplesCommon::gLogError << "[BufferManager] Unsupported DataType for tensor: " << name
                                 << std::endl;
        continue;
      }
      ...
      if (vol <= 0) {
        samplesCommon::gLogWarning << "[BufferManager] Non-positive volume for " << name
                                   << ", set to 1." << std::endl;
        vol = 1;
      }
      ...
      ASSERT_TRUE(cudaMallocHost(&tm.h, tm.bytes));
      ASSERT_TRUE(cudaMalloc(&tm.d, tm.bytes));
      std::memset(tm.h, 0, tm.bytes);
      ASSERT_TRUE(cudaMemset(tm.d, 0, tm.bytes));
