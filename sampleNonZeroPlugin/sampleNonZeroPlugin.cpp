...
//! Command: ./sample_non_zero_plugin [-h or --help] [-d=/path/to/data/dir or
...
void nonZeroIndicesHelper(nvinfer1::DataType type, void const *X, void *indices,
                          void *count, void const *K, int32_t R, int32_t C,
                          bool rowOrder, cudaStream_t stream) {
  if (type == nvinfer1::DataType::kFLOAT) {
    nonZeroIndicesImpl<float>(
        static_cast<float const *>(X), static_cast<int32_t *>(indices),
        static_cast<int32_t *>(count), static_cast<int32_t const *>(K), R, C,
        rowOrder, stream);
  } else if (type == nvinfer1::DataType::kHALF) {
    nonZeroIndicesImpl<half>(
        static_cast<half const *>(X), static_cast<int32_t *>(indices),
        static_cast<int32_t *>(count), static_cast<int32_t const *>(K), R, C,
        rowOrder, stream);
  } else {
...
class NonZeroPlugin : public IPluginV3,
  ...
  void initFieldsToSerialize() {
    ...
    mDataToSerialize.emplace_back(
        PluginField("rowOrder", &mRowOrder, PluginFieldType::kINT32, 1));
  ...
  bool supportsFormatCombination(int32_t                        pos,
    ...
    if (pos == 0) {
      typeOk = inOut[0].desc.type == DataType::kFLOAT ||
               inOut[0].desc.type == DataType::kHALF;
    } else if (pos == 1) {
  ...
  int32_t getOutputShapes(DimsExprs const *inputs, int32_t nbInputs,
    ...
    auto optValue = exprBuilder.operation(
        DimensionOperation::kFLOOR_DIV, *upperBound, *exprBuilder.constant(2));
    auto numNonZeroSizeTensor =
        exprBuilder.declareSizeTensor(1, *optValue, *upperBound);
  ...
  int32_t enqueue(PluginTensorDesc const *inputDesc,
    ...
    if (!(type == nvinfer1::DataType::kHALF ||
      ...
      sample::gLogError << "Unsupported: Sample only supports DataType::kHALF "
                           "and DataType::FLOAT"
                        << std::endl;
      return -1;
    }
    ...
    if (!mRowOrder) {
      ...
      // indices at the correct places. Therefore, we will launch the kernel
      // twice: first, only to calculate the total non-zero count, which will be
      // stored in workspace; and then to actually write the non-zero indices to
      // the outputs[0] buffer.
      cudaMemsetAsync(workspace, 0, sizeof(int32_t), stream);
      nonZeroIndicesHelper(type, inputs[0], nullptr, workspace, 0, R, C,
                           mRowOrder, stream);
      nonZeroIndicesHelper(type, inputs[0], outputs[0], outputs[1], workspace,
                           R, C, mRowOrder, stream);
    } else {
      nonZeroIndicesHelper(type, inputs[0], outputs[0], outputs[1], 0, R, C,
                           mRowOrder, stream);
    }
...
class NonZeroPluginCreator : public nvinfer1::IPluginCreatorV3One {
  ...
  NonZeroPluginCreator() {
    ...
    mPluginAttributes.emplace_back(
        PluginField("rowOrder", nullptr, PluginFieldType::kINT32, 1));
...
//!
//! \details The plugin is able to output the non-zero indices in row major or
//! column major order
//!
...
//! engine
//!
//! \details This function creates a network containing a NonZeroPlugin and
//! builds
...
//!
//! \brief Creates a network with a single custom layer containing the NonZero
//! plugin and marks the
//!        output layers
//!
...
//!
bool SampleNonZeroPlugin::constructNetwork(
  ...
  auto pluginCreator = static_cast<IPluginCreatorV3One *>(
      getPluginRegistry()->getCreator("NonZeroPlugin", "0", ""));
  auto plugin = std::unique_ptr<IPluginV3>(pluginCreator->createPlugin(
      "NonZeroPlugin", &pfc, TensorRTPhase::kBUILD));
...
bool SampleNonZeroPlugin::infer() {
  ...
  // Since the data dependent output size cannot be inferred from the engine
  // denote a sufficient size for the corresponding output buffer (along with
  // the rest of the I/O tensors)
  std::vector<int64_t> ioVolumes = {mInputDims.d[0] * mInputDims.d[1],
                                    mInputDims.d[0] * mInputDims.d[1] * 2, 1};
...
//! \brief Reads the input and stores the result in a managed buffer
//!
bool SampleNonZeroPlugin::processInput(
    samplesCommon::BufferManager const &buffers) {
  ...
  samplesCommon::readPGMFile(
      samplesCommon::locateFile(std::to_string(number) + ".pgm",
                                mParams.dataDirs),
...
//! \brief Verify result
//!
//! \return whether the output correctly identifies all (and only) non-zero
//! elements
//!
bool SampleNonZeroPlugin::verifyOutput(
    samplesCommon::BufferManager const &buffers) {
  float *input =
      static_cast<float *>(buffers.getHostBuffer(mParams.inputTensorNames[0]));
...
void printHelpInfo() {
  ...
  std::cout << "--columnOrder   Run plugin in column major output mode."
            << std::endl;
}
...
int main(int argc, char **argv) {
  ...
  sample::gLogInfo
      << "Building and running a GPU inference engine for NonZero plugin"
      << std::endl;
