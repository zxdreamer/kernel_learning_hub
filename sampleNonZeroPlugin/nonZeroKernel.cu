...
inline __device__ int32_t isZero(float const &a) { return a == 0.F; }
...
template <typename T>
__global__ void findNonZeroIndicesKernel(T const *X, int32_t *indices,
                                         int32_t *count, int32_t const *K,
                                         int32_t R, int32_t C,
                                         int32_t rowOrder) {
  ...
  if (col < C) {
    for (int32_t row = 0; row < R; ++row) {
      if (!isZero(X[row * C + col])) {
        int32_t index = atomicAdd(
            count, 1); // Increment count atomically and get the previous value
        if (indices) {
          if (rowOrder == 0) {
            indices[index]      = row;
            indices[index + *K] = col;
...
template <typename T>
void nonZeroIndicesImpl(T const *X, int32_t *indices, int32_t *count,
                        int32_t const *K, int32_t R, int32_t C, bool rowOrder,
                        cudaStream_t stream) {
...
#define NONZERO_SPECIALIZED_IMPL(T)                                   \
  template void nonZeroIndicesImpl<T>(                                \
      T const *X, int32_t *indices, int32_t *count, int32_t const *K, \
